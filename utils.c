...
bool hostname_to_ip (const char *hostname, char *ip) {
    ...
    // hints.ai_flags = AI_ADDRCONFIG; // optionnel : ignore les familles non configurees localement
    ...
    for (struct addrinfo *p = res; p != NULL; p = p->ai_next) {
        ...
        if (inet_ntop(family, addr, buf, sizeof(buf)) != NULL) {
            // Precondition: 'ip' a au moins INET6_ADDRSTRLEN octets
            strcpy(ip, buf);
            ok = true;
            break;
        }
    ...
    if (!ok) {
        // Soit aucune adresse retournee, soit échec de conversion
